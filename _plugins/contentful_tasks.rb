...
require 'yaml'
require 'fileutils'
require './_plugins/mappers/asset_renderer.rb'
...
if (ENV['CONTENTFUL_PROCESSING']=='true') 
	puts "Downloading blog posts from Contenful..."
	contentful_config = Jekyll.configuration['contentful']
	puts Jekyll.env
	Jekyll::Commands::Contentful.process([], {}, contentful_config)

	puts "Generating _posts/ from _data/contentful/"
	PiqueCul::generate_posts

	puts "Downloading images into assets_contentful/"
	PiqueCul::download_assets
else
  puts 'Contentful processing disabled'
end